...
export class RabbitMqUserCreatedConsumer {
  ...
  async start(): Promise<void> {
    ...
    await this.channel.assertQueue(QUEUE_USER_CREATED_STOCK, { durable: true });
    await this.channel.bindQueue(
      QUEUE_USER_CREATED_STOCK,
      EXCHANGE_USER_EVENTS,
      "user_created"
    );
    await this.channel.assertQueue(QUEUE_USER_CREATED_STOCK_FAILED, { durable: true });

    await this.channel.consume(QUEUE_USER_CREATED_STOCK, async (msg: ConsumeMessage | null) => {
      ...
      try {
        ...
        if (count >= MAX_RETRIES) {
          ...
          this.channel.sendToQueue(QUEUE_USER_CREATED_STOCK_FAILED, msg.content, { headers });
