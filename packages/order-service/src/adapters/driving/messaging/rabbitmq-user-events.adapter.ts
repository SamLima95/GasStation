...
export class RabbitMqUserEventsAdapter implements IEventConsumer {
  ...
  onUserCreated(handler: (payload: UserCreatedPayload) => Promise<void>): void { this.handler = handler; }

  async start(): Promise<void> {
    if (!this.handler) throw new Error("Registre o handler com onUserCreated() antes de start()");
    const connection = await amqp.connect(this.rabbitmqUrl, { timeout: RabbitMqUserEventsAdapter.CONNECT_TIMEOUT_MS });
