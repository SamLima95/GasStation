...
const userCreatedPayloadSchema = z.object({
  ...
  email: z.string().min(1).transform((s) => s.trim().toLowerCase()).refine((s) => s.length <= 254).refine((s) => EMAIL_FORMAT.test(s), "Invalid email"),
  name: nameSchema,
...
export class RabbitMqUserCreatedConsumer {
  ...
  async start(): Promise<void> {
    ...
    await this.channel.assertQueue(QUEUE_USER_CREATED_FINANCIAL, { durable: true });
    await this.channel.bindQueue(QUEUE_USER_CREATED_FINANCIAL, EXCHANGE_USER_EVENTS, "user_created");
    await this.channel.assertQueue(QUEUE_USER_CREATED_FINANCIAL_FAILED, { durable: true });

    await this.channel.consume(QUEUE_USER_CREATED_FINANCIAL, async (msg: ConsumeMessage | null) => {
      ...
      try {
        ...
        if (count >= MAX_RETRIES) {
          logger.error({ err, retries: count }, "UserCreated sent to failed queue");
          this.channel.sendToQueue(QUEUE_USER_CREATED_FINANCIAL_FAILED, msg.content, { headers: { ...msg.properties?.headers, [RETRY_HEADER]: count } });
