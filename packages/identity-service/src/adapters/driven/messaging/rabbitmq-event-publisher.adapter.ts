...
import { AUDIT_EVENT, EXCHANGE_AUDIT_EVENTS, EXCHANGE_USER_EVENTS } from "@lframework/shared";
...
export class RabbitMqEventPublisherAdapter implements IEventPublisher {
  ...
  async publish(eventName: string, payload: object): Promise<void> {
    ...
    const exchange = eventName === AUDIT_EVENT ? EXCHANGE_AUDIT_EVENTS : EXCHANGE_USER_EVENTS;
